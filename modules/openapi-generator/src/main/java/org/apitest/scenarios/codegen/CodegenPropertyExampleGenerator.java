...
public class CodegenPropertyExampleGenerator {
    ...
    public CodegenPropertyExampleGenerator(DefaultCodegen codegen) {
	...
	public Object generateExamples(CodegenProperty codegenProperty, Schema schema, boolean isValid) {
		Object example = exampleFromStringOrArraySchema(codegenProperty, schema, isValid);
		if (example == null) {
			example = toExampleValue(codegenProperty, schema, isValid);
		}
        return example;
	}
	
	private String toExampleValue(CodegenProperty codegenProperty, Schema schema, boolean isValid) {

		String modelName = this.getModelName(schema);

        return toExampleValueRecursive(codegenProperty, modelName, schema, 1, "", 0, Sets.newHashSet(), isValid);
    }
	
	private Object exampleFromStringOrArraySchema(CodegenProperty codegenProperty, Schema schema, boolean isValid) {
        ...
        if (this.simpleStringSchema(schema)) {
        	StringParameterGenerator pg = new StringParameterGenerator(codegenProperty);
    ...
    private String toExampleValueRecursive(CodegenProperty root ,String modelName, Schema schema, int indentationLevel, String prefix, Integer exampleLine, Set<Schema> seenSchemas, boolean isValid) {
        ...
        if (null != schema.get$ref()) {
			...
			DateParameterGenerator pg = new DateParameterGenerator(root);
        	...
        	//CodegenProperty property = this.codegen.fromProperty("inner", schema);  
        	DateTimeParameterGenerator pg = new DateTimeParameterGenerator(root);
        	...
        	//CodegenProperty property = this.codegen.fromProperty("inner", schema);  
        	FileParameterGenerator pg = new FileParameterGenerator(root);
        	...
        	//CodegenProperty property = this.codegen.fromProperty("inner", schema);  
        	StringParameterGenerator pg = new StringParameterGenerator(root);
        	...
        	//CodegenProperty property = this.codegen.fromProperty("inner", schema);  
        	IntegerParameterGenerator pg = new IntegerParameterGenerator(root);
    		...
    		NumberParameterGenerator pg = new NumberParameterGenerator(root);
            ...
            if (ls != null && !ls.isEmpty()) {
                return fullPrefix + toExampleValue(root, schema, isValid) + closeChars;
            }
            return fullPrefix + closeChars;
    ...
    private String exampleForObjectModel(CodegenProperty root, Schema schema, String fullPrefix, String closeChars, CodegenProperty discProp, int indentationLevel, int exampleLine, String closingIndentation, Set<Schema> seenSchemas, boolean isValid) {
        ...
        for (Map.Entry<String, Schema> entry : requiredAndOptionalProps.entrySet()) {
            ...
            if (discProp != null && propName.equals(discProp.name)) {
                ...
                propExample = exampleFromStringOrArraySchema(root, schema, isValid);
            }
