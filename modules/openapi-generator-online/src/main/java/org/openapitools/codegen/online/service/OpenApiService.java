...
public class OpenApiService {
	...
	private static final String CHAT_COMPLETION_ENDPOINT = "/openai/deployments/{0}/completions?api-version=2023-05-15";
	...
	public OpenApiService(WebClient.Builder webClientBuilder) {
		this.webClient = webClientBuilder.baseUrl("https://apitesting.openai.azure.com").build();
		this.client = new OpenAIClientBuilder().credential(new AzureKeyCredential(OPENAI_API_KEY)).endpoint(ENDPOINT)
				.buildClient();
	...
	public ResponseEntity<APIRequestInsight> getApiTestInsight(APIRequest apiRequest) {
		...
		if (StringUtils.isNotEmpty(apiRequest.prompt)) {
			...
			userPrompt = mf.format(new Object[] { apiRequest.testSummary, apiRequest.expectedResponseCode,
					apiRequest.responseCode, apiRequest.apiPath, apiRequest.requestPayload, apiRequest.response,
					apiRequest.swagger, apiRequest.implementationCode, apiRequest.schemaValidationErrors });
...
//        	userPrompt += "I need to get the recommendation and modified complete swagger as only json output";
		...
		ChatCompletions chatCompletions = client.getChatCompletions(engine, new ChatCompletionsOptions(chatMessages));

		System.out.printf(Locale.ROOT, "Model ID=%s is created at %s.%n", chatCompletions.getId(),
				chatCompletions.getCreatedAt());
	...
	public Flux<ServerSentEvent<String>> streamApiTestInsight(APIRequest apiRequest) throws JsonProcessingException {
        ...
        Flux<String> openAiSseEvents =  webClient.post()
                .uri(url)
		...
		// Map Azure OpenAI events to ServerSentEvent
        Flux<ServerSentEvent<String>> sseEvents = openAiSseEvents
                .map(event -> ServerSentEvent.<String>builder().data(event).build());

        // Add a delay to simulate events over time (you can remove this in a real scenario)
        return sseEvents.delayElements(Duration.ofSeconds(1));
